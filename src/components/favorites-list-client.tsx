...
import { getFullFavoriteApartments } from "@/lib/data-client";
...
import { isPriceInRange, parsePriceRange } from "@/lib/price-range";
...
export default function FavoritesListClient({
  ...
  const [allFavoriteApartments, setAllFavoriteApartments] = useState<
    (Apartment & { isFavorited?: boolean })[]
  >([]);
  const [isLoading, setIsLoading] = useState(true);

  // Read filter params from URL
  const { q: query, district, price, roomType, sort } = searchParams;
  ...
  useEffect(() => {
    async function fetchFavorites() {
      if (user) {
        ...
        const apartmentsWithFavStatus = apartments.map((apt) => ({
          ...apt,
          isFavorited: true,
        }));
  ...
  const filteredAndSortedApartments = useMemo(() => {
    ...
    if (query) {
      ...
      filtered = filtered.filter(
        (apt) =>
          removeVietnameseTones(apt.title.toLowerCase()).includes(
            normalizedQuery,
          ) ||
          removeVietnameseTones(apt.sourceCode.toLowerCase()).includes(
            normalizedQuery,
          ),
      );
    }
    ...
    if (price) {
      const parsedRange = parsePriceRange(price);
        ...
        {isLoading ? (
          <div className="grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-3 xl:gap-8">
            {[...Array(3)].map((_, i) => (
              <div
                key={i}
                className="space-y-4 p-4 rounded-lg border animate-pulse"
              >
